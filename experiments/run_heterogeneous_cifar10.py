...
from datasets.heterogeneous_cifar10 import HeterogeneousCIFAR10
from models.cnn import CIFAR10CNN, HeterogeneousCNN
...
def main(args):
    ...
    # Prepare dataset
    print("Preparing Heterogeneous CIFAR-10 dataset...")
    dataset = HeterogeneousCIFAR10(
        num_clients=args.num_clients, 
        num_labels_per_client=args.num_labels_per_client
    )
    ...
    if args.visualize_graph:
        visualize_graph(graph, title=f"Heterogeneous CIFAR-10 - {args.graph_type} Graph")
    ...
    for round_idx in range(args.num_rounds):
        ...
        for client_id in range(args.num_clients):
            # Set optimizer with momentum for CIFAR-10
    ...
    if args.save_results:
        model_type = "hetero" if args.heterogeneous_models else "homo"
        save_path = f"results/sheaf_fmtl_cifar10_{model_type}_gamma{args.gamma}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Sheaf-FMTL on Heterogeneous CIFAR-10")
    
    # Dataset parameters
    parser.add_argument('--num_clients', type=int, default=30, help='Number of clients')
    parser.add_argument('--num_labels_per_client', type=int, default=5, 
                        help='Number of labels per client')
    
    # Model parameters
    parser.add_argument('--heterogeneous_models', action='store_true',
                        help='Use heterogeneous model architectures')
    ...
    # Training parameters
    parser.add_argument('--num_rounds', type=int, default=150, help='Number of communication rounds')
